...
<view class="coupon_tab">
  <view @tap="get_mycoupon" data-type="0" :class="'coupon_tab_li ' + (status==0?'active':'')">
    <text class="xian"></text>
    <text class="name">未使用({{not_used}})</text>
  </view>
  <view @tap="get_mycoupon" data-type="1" :class="'coupon_tab_li ' + (status==1?'active':'')">
    <text class="xian"></text>
    <text class="name">已使用({{use}})</text>
  </view>
  <view @tap="get_mycoupon" data-type="2" :class="'coupon_tab_li ' + (status==2?'active':'')">
    <text class="xian"></text>
...
<block v-if="status==0">
  <block v-for="(item, index) in coucon_list" :key="index">
    ...
    <view class="coupon_cont">
        ...
        <view class="line_cont">
          <view class="line_cont_left">
            ...
            <view class="cont_left_title">
              <text class="cont_price"><text class="cont_symble">￥</text>{{item.deduct}}</text>
              <text class="cont_use">满{{item.enough}}元使用</text>
            </view>
            <view class="cont_bottom">
              <text class="cont_time">{{item.timestart}}~{{item.timeend}}</text>
              <image v-if="item.desc!=''" :data-id="item.id" @tap="coupon_show" class="click_hide" :src="item.coupon_img"></image>
            </view>
          </view>
        </view><!--line_cont-->
      ...
      <view :style="'display:' + item.show" v-if="item.desc!=''" class="hide_info">
    ...
    <view class="coupon_line coupon_line_none">
      ...
      <view class="line_cont_right">
        <view class="coupon_button">
          <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/usesd.png"></image>
        </view>
      </view> 
    </view><!--coupon_line-->
    <view :style="'display:' + item.show" v-if="item.desc!=''" class="hide_info">
  ...
  <block v-for="(item, index) in coucon_list" :key="index">
    <view class="coupon_cont">
      ...
      <view class="coupon_line coupon_line_none">
        ...
        <view class="line_cont_right">
          <view class="coupon_button">
            <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/dated.png"></image>
          </view>
        </view> 
      </view><!--coupon_line-->
      <view :style="'display:' + item.show" v-if="item.desc!=''" class="hide_info">
  ...
  methods: {
        ...
        success: function (res) {
          ...
          if (res.data.code == 1) {
            for (var m = 0; m < res.data.data.length; m++) {
              ...
              res.data.data[m].coupon_img = "http://wximage.shedongyun.com/sdo2o/coupon_down.png";
            }
    ...
    coupon_show: function (e) {
      ...
      for (var m = 0; m < tempCouponList.length; m++) {
        if (tempCouponList[m].id == currId && tempCouponList[m].show == "none") {
          tempCouponList[m].show = "block";
          tempCouponList.coupon_img = "http://wximage.shedongyun.com/sdo2o/coupon_up.png";
        } else if (tempCouponList[m].id == currId && tempCouponList[m].show == "block") {
          tempCouponList[m].show = "none";
          tempCouponList[m].coupon_img = "http://wximage.shedongyun.com/sdo2o/coupon_down.png";
        }
      }
