...
<view class="body">
  <view class="order_static">
    <image src="http://wximage.shedongyun.com/sdo2o/back_detail.png" class="back_detail"></image>
    <view class="detail_main" v-if="order_info.order_status!='' || order_info.order_status!=undefind">
      <view class="detail_cont">
        <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/car_send.png" class="status_type" v-if="order_info.order_desc=='商家取件中' || order_info.order_desc=='商家配送中'"></image>
        <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/car_state.png" class="status_type" v-else-if="order_info.order_desc=='等待商家上门取件'"></image>
        <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/car_clear.png" class="status_type" v-else-if="order_info.order_desc=='商家清洗中'"></image>
        <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/car_finish.png" class="status_type finish_status" v-else-if="order_info.order_desc=='订单交易完成'"></image>
        <view class="status">{{order_info.order_status}}</view>
        <view class="status_t" v-if="order_info.order_status_flag==1">请在{{gradeTime}}分{{countDownNum}}秒完成支付，超时将取消订单</view>
  ...
  <view class="borderh4"></view>
    <block v-for="(item, index) in order_info.list" :key="index" v-if="order_info.list!=null">
      <view class="title">
        <view class="title_date">
          <image src="http://wximage.shedongyun.com/sdo2o/order_merch.png" class="store_picture"></image>
          <text class="store_name">{{item.merchname}}</text>
          <image src="http://wximage.shedongyun.com/sdo2o/order_skip.png" class="skip_right"></image>
        </view>
      </view>
          ...
          <view class="goods_list">
            ...
            <view class="goods_one">
              ...
              <view class="goods_price">
                <text class="goods_number"><text class="symbol">￥</text>{{list_item.marketprice}}</text>
                <text class="real_name">标准服务</text>
              </view>
              <text class="goods_unit"><text class="unit_symbol">X</text>1</text>
    ...
    <view class="amount_real">
      <view class="amount_main">
        ...
        <text class="amount_second"><text class="money_symble">￥</text>{{order_info.price}}</text>
      </view>
      <view class="operate_main">
        ...
        <view class="operate_right" @tap="phone_call" :data-phone="order_info.shop_phone">
          <image src="http://wximage.shedongyun.com/sdo2o/phone_detail.png" class="phone_detail"></image>
    ...
    <view class="detail_style">
      ...
      <view class="detail_line">
        ...
        <text class="line_right">{{order_info.address_info.province}}{{order_info.address_info.city}}{{order_info.address_info.area}}</text>
      </view>
      <view class="detail_line bottom_line">
        <text class="line_left">联系人:</text>
        <text class="line_right">{{order_info.address_info.realname}}  {{order_info.address_info.mobile}}</text>
      </view>
      <view class="detail_line">
    ...
    <view class="borderh4"></view>
    <view class="complaint_main" @tap="phone_call" :data-phone="order_info.shop_phone">
      <text class="complaint_name">投诉商家</text>
      <image src="http://wximage.shedongyun.com/sdo2o/clock_right.png" class="phone_right"></image>
    </view>
    <view class="borderh4"></view>
    <view class="borderh4"></view>
    <view class="borderh4"></view>
    ...
    <view class="order_box">
      ...
      <view class="dele_oder">
        <navigator v-if="order_info.status==0" @tap="order_edit" :data-order_id="order_info.id" data-edit="order_pay" hover-class="none" class="immed_pay">立即支付</navigator>

        <navigator v-if="order_info.status==3" @tap="order_evaluat" :data-order_id="order_info.id" hover-class="none" class="immed_pay">去评价</navigator>

        <navigator v-if="order_info.order_status_flag==2" hover-class="none" class="immed_pay" :url="'../../../pages/refund/refund_apply/refund_apply?order_id=' + order_info.id">申请退款</navigator>

        <navigator v-if="order_info.order_status_flag==3" hover-class="none" :url="'../../../pages/refund/refund_apply/refund_apply?order_id=' + order_info.id">退款申请</navigator>

        <navigator v-if="order_info.order_status_flag==7" hover-class="none" class="cancel_order" @tap="order_received" :data-order_id="order_info.id">确认收货</navigator>
        
        <navigator v-if="order_info.status==0" @tap="order_cancel" :data-order_id="order_info.id" data-edit="order_pay" class="cancel_order" hover-class="none">取消订单</navigator>
        ...
        <navigator v-if="order_info.status==1  && order_info.refundstate==1" @tap="abolish_cancel" hover-class="none">取消申请</navigator>
        <!-- <navigator wx:if="{{order_info.status==4}}" bindtap='abolish_cancel' hover-class='none'>取消申请</navigator> -->
        <navigator v-if="order_info.status==2 && order_info.refundstate==1" @tap="abolish_cancel" hover-class="none">取消申请</navigator>
...
export default {
  ...
  methods: {
    ...
    gologistic: function (e) {
      console.log(e);
      wx.navigateTo({
        url: '../../../pages/user/Logistics/Logistics?orderid=' + e.currentTarget.dataset.id
      });
    },
    // 退款退货订单申请
    ...
    order_cancel: function (e) {
      ...
      wx.redirectTo({
        url: '/pages/user/order_cancel/order_cancel?openid=' + openid + '&orderid=' + orderid
      });
    },
            ...
            if (res.data.data.order_is_receive == 1) {
              wx.showToast({
                ...
                success: function () {
                  setTimeout(function () {
                    wx.navigateTo({
                      url: '/pages/user/order_list/order_list?order_status=complete'
                    });
                  }, 2000);
                }
              });
