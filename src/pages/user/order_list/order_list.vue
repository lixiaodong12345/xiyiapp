...
<view class="body">
  <view class="navswich">
    ...
    <view class="one_block" @tap="nav_switch" data-nav="payment">
      <text :class="order_status == 'payment'?'current_nav':''">待付款</text>
    </view>
    ...
    <view class="one_block" @tap="nav_switch" data-nav="shipped">
      <text :class="order_status == 'shipped'?'current_nav':''">待服务</text>
    </view>
    <view class="one_block" @tap="nav_switch" data-nav="complete">
      <text :class="order_status == 'complete'?'current_nav':''">已完成</text>
    </view>
  ...
  <view class="scroll_view">
    ...
    <block v-for="(item, index) in order_list" :key="index" v-if="order_list !=null">
      <view class="list">
        <!--头部状态-->
        <view class="title">
          <view class="title_date">
            ...
            <image src="http://wximage.shedongyun.como2o/order_merch.png" class="store_picture"></image>
          ...
          <view class="wait_goods">
            ...
            <block v-else>
              ...
              <text v-if="item.status == 1 || (item.status == 0 && item.paytype == 3)">待接单</text>
              <text v-if="item.status == -1 && item.refundtime == 0">已取消</text>
        ...
        <view class="goods_list">
          <navigator v-if="item.list==null" hover-class="none">
            ...
            <block v-for="(goods, index2) in item.goods_list" :key="index2">
              <view class="goods_one" @tap="order_detail" :data-order_id="item.id">
                ...
                <view class="name_number">
                  ...
                  <view class="goods_price">
                    <text class="goods_number"><text class="symbol">￥</text>{{goods.price}}</text>
                    <text class="real_name">标准服务</text>
                  </view>
                  <text class="goods_unit"><text class="unit_symbol">X</text>{{goods.total}}</text>
          ...
          <view class="order_inform">
            ...
            <view class="order_addre">
              <image src="http://wximage.shedongyun.com/sdo2o/order_addre.png" class="order_adder"></image>
              <text class="addre_name">{{item.address}}</text>
            </view>
          </view>
          <view v-if="item.status == 0 && item.paytype != 3" class="goods_list_bot">
            <button @tap="order_edit" :data-order_id="item.id" data-edit="order_pay" class="true">去支付</button>
            <button :data-order_id="item.id" @tap="order_cancel" class="false">取消订单</button>
          </view>
          <view v-if="item.status == 3" class="goods_list_bot">
            <button @tap="order_evaluat" :data-order_id="item.id" class="true">去评价</button>
          </view>
          <!-- <view wx:if="{{item.status == 2}}" class="goods_list_bot">
            <button data-order_id="{{item.id}}" bindtap='order_cancel' class='false'>取消订单</button>
          </view> -->
          <view v-if="item.status == 4 || item.status == 5 || item.status == 6 || item.status == 2" class="goods_list_bot">
            <button :data-order_id="item.id" @tap="order_detail" class="false">查看信息</button>
          </view>
          <view v-if="item.status == 7" class="goods_list_bot">
            <button :data-order_id="item.id" @tap="order_received" class="false">确认收货</button>
          </view>
        </view><!--商品列表及订单状态-->
      </view>
...
export default {
  ...
  methods: {
    ...
    comment_order: function (e) {
      if (e.target.dataset.order_id) {
        wx.navigateTo({
          url: '/pages/user/order_comment/order_comment?order_id=' + e.target.dataset.order_id
        });
      }
    },
    // 立即付款
