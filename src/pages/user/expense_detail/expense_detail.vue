...
<view class="container_wrap">
  ...
  <view class="select_option">
    <view class="select_one" @tap="startshow">
      ...
      <image mode="widthFix" src="http://wximage.shedongyun.com/sdo2o/detail_down.png" class="select_down"></image>
    </view>
  ...
  <view :class="'search_down_cont ' + (giantView=='false'?'hide':'')">  
    ...
    <view class="search_cont_font">
      <view @tap="tabSelect" data-select="pro">
        <image src="http://wximage.shedongyun.com/sdo2o/search_prod.png" class="search_icon"></image>
        <text class="select_nav">消费</text>
      </view>
      <view @tap="tabSelect" data-select="store">
        <image src="http://wximage.shedongyun.com/sdo2o/search_mearch.png" class="search_mearch"></image>
      ...
      <view class="particul_year">
        ...
        <view class="particul_main">
          ...
          <view class="particul_line" v-if="oneItem.status == 1">
            <image src="http://wximage.shedongyun.com/sdo2o/exp_out.png" class="particul_logo"></image>
          ...
          <view class="particul_line" v-else-if="oneItem.status == 2">
            <image src="http://wximage.shedongyun.com/sdo2o/exp_into.png" class="particul_logo"></image>
            <view class="particul_inform">
              <view class="inform_title">{{oneItem.title}}</view>
              <text class="inform_use">[充值]</text>
              <view class="inform_time">{{oneItem.createtime}}</view>
              <view class="inform_money red_color">+{{oneItem.price}}</view>
            </view>
          </view><!--particul_line-->
          </block>
        </view>
...
<view class="starttime" :style="startpop">
  <view class="time_box">
    <picker-view indicator-style="height: 50px;" class="select_wrap" :value="value" @change="startChange">
      <picker-view-column>
        <view v-for="(item, index) in years" :key="index" class="select_line">{{item}}年</view>
      </picker-view-column>
      <picker-view-column>
        <view v-for="(item, index) in months" :key="index" class="select_line">{{item}}月</view>
      </picker-view-column>
    </picker-view>
  </view>
