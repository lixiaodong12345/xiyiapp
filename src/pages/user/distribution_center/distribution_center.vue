...
<view v-if="shopBanner" class="banner_wrap">
  <swiper class="banner" indicator-dots="true" :indicator-color="indicator_color" :indicator-active-color="indicator_active_color" :autoplay="autoplay" :circular="circular" :interval="interval" :duration="duration">
...
<view class="notice_wrap">
  <view class="notice_cont">
    <view class="notice_horn">
      <image src="http://wximage.shedongyun.com/sdo2o/horn_logo.png" class="notice_title"></image>
    </view>
    <swiper :autoplay="autoplay" :interval="noticeInterval" :duration="notictDuration" vertical="true" :circular="circular" class="notice_swiper">
...
<!--商家入驻-->
<image src="http://wximage.shedongyun.com/sdo2o/merchant_in.png" @tap="merchant_skip" class="merchant_in"></image>
<!--、快捷菜单栏模块-->
<view class="menu_wrap">
  <view class="menu_cont">
    <!--块 少于10个-->
    <view class="first_memu">
      <block v-for="(item, index) in menuList" :key="index" v-if="index<10">
        <view class="menu_main" @tap="common_skip" :data-link="item.link_wxapp">
...
<view class="large_module">
  ...
  <view class="module_title">
    <text :class="'one_module ' + (navBar==0?'current_title':'')" data-type="distance" @tap="bar_switch" data-bar="0">附近</text>
    <text :class="'one_module ' + (navBar==1?'current_title':'')" data-type="time" @tap="bar_switch" data-bar="1">新入住</text>
    <text :class="'one_module ' + (navBar==2?'current_title':'')" data-type="hot" @tap="bar_switch" data-bar="2">热门</text>
  </view>
  <!--列表-->
      ...
      <block v-for="(item, index) in shopList" :key="index">
        <view class="module_line">
          ...
          <view class="module_main">
            <navigator :url="'/pages/shop/shop_detail/shop_detail?id=' + item.id" hover-class="none">
              <view class="shop_title" :data-id="item.id">{{item.merchname}}</view>
              ...
              <view class="module_clock module_posit" v-if="item.address!=''">
                ...
                <text class="dist_addre" v-if="item.distance!=0">{{item.distance}}km</text>
                <image src="http://wximage.shedongyun.com/sdo2o/right_logo.png" class="right_logo"></image>
              </view>
            </navigator>
            <!--电话-->
            <image @tap="collphone" :data-tel="item.mobile" src="http://wximage.shedongyun.com/sdo2o/phone_logo.png" class="phone_logo"></image>
          </view>
        </view>
      </block>
      <!--加载更多-->
...
export default {
  ...
  methods: {
    ...
    common_skip: function (e) {
      ...
      if (link_wxapp == "/pages/index" || link_wxapp == "/pages/user/distribution_center/distribution_center" || link_wxapp == "/pages/user/index") {
        wx.switchTab({
          url: link_wxapp
        });
      } else {
