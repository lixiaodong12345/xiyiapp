...
<view v-if="team_list">
  <block v-for="(item, index) in team_list" :key="index">
    <view class="wechat_team" v-if="item.isagent==1">
        ...
        <view class="wechat_tx1">
            ...
            <text class="avatar_number_tx2_02">成为分销商时间:{{item.agenttime}}</text>
            <text class="avatar_number_tx2">{{item.team_member_count}}个成员</text>
        </view>
     </view>
      <view class="wechat_team_01" v-if="item.isagent==0">
          ...
          <view class="wechat_tx1_01">
              <text class="avatar_number_tx1_01">{{item.nickname}}</text>
              <text class="avatar_number_tx2_02">注册时间:{{item.createtime}}</text>
  ...
  <view class="team_none" v-if="team_list == ''">
      <image src="http://wximage.shedongyun.com/sdo2o/icon/hr.png" style="padding-left: 165rpx;"></image>
      <text>目前还没有团队</text>
