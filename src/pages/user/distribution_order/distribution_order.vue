...
<view class="oder_tab">
    <view :class="'oder_tab_child ' + (order_status =='all'?'active':'' )" data-type="button" @tap="changeOrderStatus" data-status="all">
        <text>所有</text>
    </view>
    <view :class="'oder_tab_child ' + (order_status =='payment'?'active':'' )" data-type="button" @tap="changeOrderStatus" data-status="payment">
        <text>待付款</text>
    </view>
    <view :class="'oder_tab_child ' + (order_status =='paymented'?'active':'' )" data-type="button" @tap="changeOrderStatus" data-status="paymented">
        <text>已收款</text>
    </view>
    <view :class="'oder_tab_child ' + (order_status =='complete'?'active':'' )" data-type="button" @tap="changeOrderStatus" data-status="complete">
        <text>已完成</text>
    </view>
</view>
<!--有数据时的页面-->
<scroll-view scroll-y="true" scroll-left="1" data-type="scroll" class="listbox" @scrolltolower="load_list">
  ...
  <block v-for="(item, index) in order_list" :key="index">
      <view class="team_child">
          ...
          <view class="team_child_right">
              <text v-if="item.statusname!=''" class="team_child_right_001">{{item.statusname}}</text>
          </view>
      </view>
      ...
      <block v-for="(goods, index2) in item.goods_list" :key="index2">
        <view class="wechat_name">
            <view class="user_name_all">
                <image class="userinfo-avatar_001" :src="goods.thumb" background-size="cover"></image>
                <view class="goods_info">
                    <text class="goods_name">商品名称{{goods.title}}</text>
                    <text class="wechat_name_tx01">预计佣金:<text class="money">+{{goods.goods_commissions}}</text></text>
                </view>
      ...
      <view class="team_child">
          ...
          <view class="team_child_right">
              <text class="team_child_right_002">微信名称：{{item.nickname}}</text>
          </view>
      </view>
      <view class="border_006"></view>
    </block>
...
<view class="team_none" v-if="order_list == ''">
    <image src="/static/static/images/icon/hr.png" style="padding-left: 165rpx;"></image>
    <text>目前还没有数据</text>
