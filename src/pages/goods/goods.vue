...
<view class="goods_body">
  ...
  <view class="goods_name">
    ...
    <view class="share_top" @tap="share_skip">
      <image src="http://wximage.shedongyun.com/sdo2o/button_share.png" class="share_icon"></image>
  ...
  <view class="play_money">
    <view v-if="goods_infosss.ispresell=='0'">
      ...
      <text v-if="goods_infosss.is_section=='0'" class="play_money_price">{{goods_info_price}}</text>
      <text v-if="goods_infosss.is_section!='0'" class="play_money_price">{{goods_infosss.price_section.min_price}}</text>
      ...
      <text v-if="goods_infosss.is_section!='0'" class="play_money_price">{{goods_infosss.price_section.max_price}}</text>
    ...
    <view v-if="goods_properties!='null'">
      ...
      <view class="select_wrap">
        <view class="select_main">
          ...
          <view class="select_wrap_right" @tap="judgeCategory" :style="specif_have">
  ...
  <view class="store_cont">
    <view class="shop_head">
      ...
      <view class="store_main">
        ...
        <text v-if="shop.goods_info.merchid!=0" @tap="attention" data-type="add" :data-merchid="shop.goods_info.merchid" :style="add" class="select_intro">{{shop.goods_info}}点击关注</text>
        <text v-if="shop.goods_info.merchid!=0" @tap="attention" data-type="cancel" :data-merchid="shop.goods_info.merchid" :style="cancel" class="select_intro">{{shop.goods_info}}取消关注</text>
      </view>
    </view>
    <!--进入店铺-->
          ...
          <view class="waiwrapper">
            ...
            <view>
              <radio-group class="attr_border">
                <label v-for="(values, index2) in item.spec_items" :key="index2" @tap="select_scalea" :id="goods_info_id" :data-id="values.id" :data-name="id" :data-ck="checkId[id]" :class="'attr_item ' + (checkId[id] == values.id?'active':'')">
      ...
      <form @submit="formSubmit" report-submit="true">
        ...
        <view class="bottom_wrap">
          ...
          <view class="Join_shopping_cart Join_shopping_cart_wrap">
            <button :id="goods_info_id" :data-goods_spec="goods_spec" :data-goods_num="goods_number" data-fast="0" data-name="add_to_cart" :data-number="protuct_number" formType="submit">加入洗衣篮</button>
          </view>
          <!--立即购买-->
          <view class="buy_now buy_now_wrap">
            <button :id="goods_info_id" :data-goods_spec="goods_spec" :data-goods_num="goods_number" data-fast="1" data-name="buy_now" :data-number="protuct_number" formType="submit">立即购买</button>
          </view>
        </view>
      </form>
    </view>
      ...
      <form @submit="formSubmit" report-submit="true">
        <view class="bottom_wrap">
          ...
          <view class="Join_shopping_cart cate_shopping_cart_wrap" :style="confirm_cart">
            <button :id="goods_info_id" :data-goods_spec="goods_spec" :data-goods_num="goods_number" data-fast="0" data-name="add_to_cart" :data-number="protuct_number" formType="submit" class="font_cart" style="background:#55bcc5">确定</button>
          </view>
          <!--立即购买-->
          <view class="buy_now cate_now_wrap" :style="confirm_buy">
            <button :id="goods_info_id" :data-goods_spec="goods_spec" :data-goods_num="goods_number" data-fast="1" data-name="buy_now" :data-number="protuct_number" formType="submit" class="font_cart" style="background:#55bcc5">确定</button>
          </view>
        </view>
      </form>
    </view>
