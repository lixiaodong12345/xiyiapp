...
<view class="container">
  <!--一个大模块-->
  <block v-for="(item, index) in cateList" :key="index" v-if="item.sub_gcates!='' && item.sub_gcates!=undefind">
    <view class="one_module">
      <view class="nav_left">
        <image src="http://wximage.shedongyun.com/sdo2o/cate01.png" class="nav_left_img"></image>
        <view class="nav_left_items">{{item.name}}</view>
      </view><!--一级导航结束-->
      <view class="nav_right">
        <view class="all_title">
          <block v-for="(item, index2) in item.sub_gcates" :key="index2">
            <view :class="'Three_level_menu ' + (item.id==currId?'current':'')" @tap="gcate_skip" :data-id="item.id">{{item.name}}</view>
          </block>
        </view>
      </view><!--二级导航结束-->
    </view><!--一个大模块结束-->
